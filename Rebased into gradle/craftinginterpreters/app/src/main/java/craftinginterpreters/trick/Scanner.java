...
class Scanner {
    ...
    */
    ...
    private void character(){
        if(peek() != '\'' && !isAtEnd()){ // Case where there is a char and it is in the middle
            advance();
        }
        else{
            if(isAtEnd()){
                Trick.error(line, "Unterminated char");     //Case where the char doesnt end
            }
            else{
                Trick.error(line, "No expression given to char"); //Case where the char is empty
                advance();
            }
            return;
        }
